...
function CourseEditPage() {
  ...
  async function handleSave() {
    ...
    const stopInputs = resolvedStops.map((stop, i) => ({
      placeId: stop.placeId,
      stopOrder: i,
      dayNumber: stop.dayNumber,
      ...(stop.stayDurationMinutes !== undefined && {
      ...
      createCourse.mutate(
        {
          ...
          safetyPriority: course.safetyPriority,
          copiedFromCourseId: course.courseId,
          stops: stopInputs,
        },
        {
          onSuccess: (created) => {
            void navigate({
              to: '/course/$courseId',
              params: { courseId: created.courseId.toString() },
            })
          },
        },
      )
  ...
  return (
    <div className="bg-background min-h-screen pb-20">
        ...
        actions={
          <button
            ...
            disabled={
              isTitleInvalid ||
              (!demoCourse &&
                (updateCourse.isPending || createCourse.isPending || createPlace.isPending))
            }
      ...
      <main className="px-margin-mobile pt-lg pb-xl mx-auto max-w-2xl">
        {!demoCourse && (updateCourse.isError || createCourse.isError || createPlace.isError) && (
